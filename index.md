---
layout: welcome
title: jyje.online
cover: true
show_language_toggle: true
---

저는 서울에서 활동하는 **AI 플랫폼 엔지니어 전제영([Jeayoung Jeon])**입니다.
기술로 문제를 풀고, 팀과 함께 성장하는 것을 추구합니다. **AI와 클러스터를 주력 도메인으로 활동하는 실무경력 6년차 소프트웨어 엔지니어**입니다.

현재 [현대오토에버]{:.heading.flip-title} 개발환경플랫폼팀에서 AI 개발을 리드하고 있습니다.
저는 다음의 전문성을 가지고 있습니다:

- 🤖 **Agentic AI 개발** — Agentic AI 앱과 인프라를 Zero to Product로 만들 수 있습니다. AI Slop이 아닌, AI-Native한 개발 문화를 만듭니다.  
- 🐳 **클라우드 네이티브 운영** — 클라우드 네이티브 환경에서 Agentic AI 앱과 AIOps, GPU 플랫폼을 구현/운영할 수 있습니다.  
- 🤝 **팀 리딩** — 10인 내외 팀 프로젝트에서 개발 협업, 아키텍트 및 플랫폼 엔지니어링을 리딩할 수 있습니다.  
- 🧑‍🎓 **기술지식** — 머신러닝, 컴퓨터비전, 차량기술 연구원 경험을 활용하여 AIOps/MLOps/DevOps 관련 의사 결정에 기여할 수 있습니다.

저는 AI와 특화 도메인의 다양한 도전을 꿈꾸고 있습니다. 기여할 수 있는 팀이 있다면 편하게 연락 부탁드립니다. 자세한 경력은 [포트폴리오]{:.heading.flip-title}를 참고해 주세요.

```card
---
- title: 이력서
  icon: icon-clipboard
  description: 경력, 성과, 자격을 담은 2페이지 요약 문서.
  link: /ko/resume
  cta: "바로가기 →"
- title: 포트폴리오
  icon: icon-briefcase
  description: 프로젝트 중심의 경력 문서.
  link: /ko/portfolio
  cta: "바로가기 →"
- title: 데모
  icon: icon-rocket
  description: 작업물 라이브 데모 (Raspberry Pi 클러스터).
  link: https://app.jyje.online
  cta: "바로가기 →"
- title: 포스트
  icon: icon-book
  description: 기술 포스팅 및 연구 게시물.
  link: /ko/posts
  cta: "바로가기 →"
- title: 성과
  icon: icon-trophy
  description: 주요 전문 활동 및 프로젝트 성과.
  link: /ko/posts/#tags=achievements
  cta: "바로가기 →"
- title: 자격증
  icon: icon-certificate
  description: 전문 자격증 및 인증 내역.
  link: /ko/posts/#tags=certifications
  cta: "바로가기 →"
- title: 블로그
  icon: icon-bubble
  description: 커리어와 기술 주제에 대한 개인 블로그.
  link: https://blog.jyje.online
  cta: "바로가기 →"
---
```

문의 또는 협업은 [LinkedIn]{:.heading}으로 연락해 주세요.

<div style="margin-top: 5rem;">
  {% include components/dingbat.html %}
</div>


## 💼 상세 경력

### 1. 조직별 경력 및 역할

현재 몰입하고 있는 분야는 **AI 애플리케이션, LLM/MLOps 플랫폼, 하이브리드 Kubernetes 클러스터, 클라우드 네이티브 AI/ML 인프라**입니다. 이전에는 **AR/디지털 트윈** 전문 기업 [MAXST]{:.heading.flip-title}에서 **MLOps/DevOps 엔지니어**로 근무하였으며, 컴퓨터 비전·자율주행·AI 분야의 연구 경험을 보유하고 있습니다.

```roadmap
---
- title: 현대오토에버
  img: /assets/img/logos/hae.png
  current: true
  period: 2025 – 현재
  subtitle: 리드 AI 플랫폼 엔지니어
  description: Agentic AI 개발그룹 빌딩 · 완성차 AI 하네스 개발 및 클라우드 아키텍트 수행 · 지식기반 및 도메인 특화 도구 개발 · LLM 서빙 및 최적화
- title: MAXST 주식회사
  img: /assets/img/logos/maxst.png
  period: 2021 – 2024
  subtitle: 리드 MLOps / DevOps 엔지니어
  description: MLOps & LLMOps · 하이브리드 클러스터 (AWS EKS + 온프레미스) · 디지털 트윈
- title: 포항공과대학교 (POSTECH)
  img: /assets/img/logos/postech.png
  period: 2012 – 2020
  subtitle: 전기전자공학과 통합 박사과정
  description: 컴퓨터 비전 · 자율주행 ADAS · Visual-SLAM & SfM · FPGA 병렬 가속
- title: 금오공과대학교
  img: /assets/img/logos/kit.png
  period: 2008 – 2012
  subtitle: 전자공학부 전자통신 학사 (B.S.)
  description: 학점 4.3/4.5 · 통신 & 디지털 신호처리 · 수중 LED 가시광 통신 연구
---
```

### 2. 역량별 경력

```expertise
---
phases:
  - id: 1
    label: Academic Foundation
    label_ko: 학문적 기초
  - id: 2
    label: Research
    label_ko: 연구
  - id: 4
    label: Engineering
    label_ko: 엔지니어링
  - id: 5
    label: Current
    label_ko: 현재
items:
  - label: 통신 & DSP
    from: 2008
    to: 2018
    phase: 1
    skills: 신호 처리 · 아날로그/디지털 통신 · MATLAB
  - label: 머신러닝
    from: 2008
    to: 2022
    phase: 1
    skills: 회귀 · 분류 · 딥러닝
  - label: 컴퓨터 비전
    from: 2012
    to: 2020
    phase: 1
    skills: 영상 처리 · CNN · 객체 탐지
  - label: FPGA & ADAS
    from: 2012
    to: 2018
    phase: 2
    skills: 병렬 컴퓨팅 · SoC · VHDL · 실시간 처리
  - label: 자율주행
    from: 2012
    to: 2020
    phase: 2
    skills: SLAM · 경로 계획 · 센서 융합
  - label: Visual-SLAM & SfM
    from: 2018
    to: 2024
    phase: 2
    skills: 3D 복원 · 맵핑 · 위치 추정
  - label: 디지털 트윈 & AR
    from: 2018
    to: 2024
    phase: 2
    skills: AR 플랫폼 · 장면 이해 · SDK
  - label: MLOps & DevOps
    from: 2021
    to: 2026
    phase: 4
    skills: CI/CD · Kubernetes · 모델 배포
  - label: 하이브리드 클러스터
    from: 2022
    to: 2026
    phase: 4
    skills: AWS EKS · 온프레미스 · GitOps · IaC
  - label: ML API & 파이프라인
    from: 2023
    to: 2026
    phase: 4
    skills: 데이터 파이프라인 · RAG · LLMOps · Vector DB
  - label: AI 플랫폼
    from: 2025
    to: 2026
    phase: 5
    skills: Agentic AI · MCP · LLM 서빙 및 최적화
---
```


### 3. 이 사이트에 대해

이 사이트는 저의 전문 프로필 페이지입니다. [이력서]{:.heading.flip-title}와 [포트폴리오]{:.heading.flip-title} 등 경력 문서, 기술과 경험을 담은 [포스트]{:.heading.flip-title}, 그리고 [자격증]{:.heading.flip-title} 기록 등을 담고 있습니다.


<div style="margin-top: 5rem;">
  {% include components/dingbat.html %}
</div>


<!-- profile -->
[Jeayoung Jeon]: https://www.linkedin.com/in/jyje "LinkedIn Profile"
[전제영(Jeayoung Jeon)]: https://www.linkedin.com/in/jyje "LinkedIn Profile"

<!-- resume -->
[이력서]: /ko/resume "my-profile resume --korean"

<!-- portfolio -->
[포트폴리오]: /ko/portfolio "my-profile portfolio --korean"

[포스트]: /ko/posts
[자격증]: /ko/posts/#tags=certifications
[Works]: /works

[Blog]: https://blog.jyje.online "My blog"
[MAXST]: https://www.linkedin.com/company/maxst "LinkedIn profile of MAXST Co., Ltd."
[Hyundai Autoever]: https://www.linkedin.com/company/hyundai-autoever "LinkedIn profile of Hyundai AutoEver"

[Mail]: mailto:jyjeon+profile@outlook.com?subject=To&nbsp;Jeayoung&nbsp;Jeon
[메일]: mailto:jyjeon+profile@outlook.com?subject=To&nbsp;Jeayoung&nbsp;Jeon

[Demo]: https://app.jyje.online "Demo"
[데모]: https://app.jyje.online "Demo"
[블로그]: https://blog.jyje.online "Blog"
[현대오토에버]: https://www.linkedin.com/company/hyundai-autoever "LinkedIn profile of Hyundai AutoEver"
[LinkedIn]: https://www.linkedin.com/in/jyje "LinkedIn profile of Jeayoung Jeon"

{% include components/author.html author=site.data.authors.jyje heading="저자 소개" %}

<div style="margin-top: 7rem;">
  {% include components/dingbat.html %}
</div>

![이 사이트의 로고](/assets/icons/icon-128x128.png){:.centered style="margin-top:10rem; margin-bottom:10rem;"}
